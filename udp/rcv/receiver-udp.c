...
int main(int argc, char **argv)
{
	 ...
	 * sender_addr contiendra les informations relatives à l'envoyeur
	 * Voir : man recv
	 */
	if(recvfrom(sockfd, buf, 1024, 0, (struct sockaddr *) &sender_addr, &addrlen) == -1) {
		perror("recvfrom");
		close(sockfd);
		exit(EXIT_FAILURE);
	}
